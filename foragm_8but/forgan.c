...
char prmt='d';
int colpin[COLN] = {4, 18, 10, 12, 27, 23, 7, 16};
int rowpin[ROWN] = {17, 22};
int settingpin[SETN] = {21, 1, 0, 20, 2, 5, 3, 6};
	   ...
	   if(chordtype>MAJ && chordtype<AUG){
		triad[1] -= 1;
	  	if(chordtype>1) triad[2] -= 1;
           ...
           else if(chordtype == PWR) triad[1] = triad[0]+0x0c;
